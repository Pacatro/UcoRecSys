...
class NeuralHybridV2(nn.Module):
    def __init__(
        self,
        n_users: int,
        n_items: int,
        ...
        # MLP - ahora solo recibe features de contenido
        n_cat = len(self.cat_embeddings)
        n_num = len(cont_features)
        mlp_input = n_cat * (emb_dim // 2) + n_num
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # Solo las features de contenido van a la MLP
        content_features = torch.cat([cat_embs, num_embs], dim=1)

        # Predicción final: producto punto + MLP(content) + biases
        if content_features.size(1) > 0:
            content_score = self.mlp(content_features).squeeze(1)
        else:
            content_score = torch.zeros_like(dot_product)

        raw = dot_product + content_score + u_b + i_b
